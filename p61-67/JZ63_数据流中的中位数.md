//朴素的方法，存入，每次都要排序，肯定不行，因此可以用优先队列
```c
class Solution {
public:
    ...
    { 
        ...
        if(count%2){
            ...
            for(int i=0;i<=count/2;i++){
                num=pq.top();pq.pop();tmp.push_back(num);
            }
            for(auto n:tmp) pq.push(n);
            return num+0.0;
        }
        else{
            int num,num1;tmp.clear();
            for(int i=0;i<=count/2;i++){
                num=pq.top();pq.pop();tmp.push_back(num);
                if(i==(count/2-1)) num1=num;
            }
            for(auto n:tmp) pq.push(n);
            return (num+num1+0.0)/2;
        }
    }
